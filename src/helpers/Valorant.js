...
const saveClient = (input) => {
    client = input;  
    // if(client.config.VALORANT.HDevToken === undefined){
    //     setTimeout(() => {
    //         VAPI = new unofficialValorantApi(client.config.VALORANT.HDevToken) // It was starting without client.config.VALORANT
    //     }, 10000);
    // }else{
    //     VAPI = new unofficialValorantApi(client.config.VALORANT.HDevToken)
    // }
    ...
    try {
        ...
        for(const id of getUserList()) {
            try {
                    ...
                    do { // retry loop in case of rate limit or maintenance
                        ...
                        if(!offers.success) {
                            if(offers.maintenance) {
                                client.logger.debug("Valorant servers are under maintenance, waiting 15min before continuing alert checks...");
                                await wait(15 * 60 * 1000);
                            }

                            else if(offers.rateLimit) {
                                const waitMs = offers.rateLimit - Date.now();
                                client.logger.error(`I got ratelimited while checking alerts for user ${id} #${i} for ${Math.floor(waitMs / 1000)}s!`);
                                await wait(waitMs);
                            }
                ...
                client.logger.error("There was an error while trying to fetch and send alerts for user " + discordTag(id));
        ...
        client.logger.error("There was an error while trying to send alerts!");
        client.logger.error(e);
    }
}

const sendAlert = async (id, account, alerts, expires, tryOnOtherShard=true, alertsLength=alerts.length) => {
    ...
    */
    ...
    for(const channel_id of Object.keys(filteredAlerts)) {
        ...
        client.logger.debug(`Sending alert for user ${username}...`);
        ...
        for(const alert of alertsArray) {
            ...
            client.logger.debug(`User ${valorantUser.username} has the skin ${l(skin.names)} in their shop!`); //only we see it, no need to see the skin name in another language
            if(alertsLength === 1){
                message.embeds.push({
        ...
        await channel.send(message).catch(async e => {
            client.logger.error(`Could not send alert message in #${channel.name}! Do I have the right role?`);

            try { // try to log the alert to the console
                const user = await client.users.fetch(id).catch(() => {});
                if(user) client.logger.error(`Please tell ${user.tag} that the skin his want is in their item shop!`); // sorry for that :(
            } catch(e) {}
...
const sendCredentialsExpired = async (id, alert, tryOnOtherShard=true) => {
    ...
    if(!channel) {
        ...
        const user = await client.users.fetch(id).catch(() => {});
        if(user) client.logger.error(`Please tell ${user.tag} that their credentials have expired, and that they should /login again. (I can't find the channel where the alert was set up)`);
        return;
    }
    ...
    await channel.send({
        ...
        client.logger.error(`Could not send message in #${channel.name}! Do I have the right role?`);

        try { // try to log the alert to the console
            const user = await client.users.fetch(id).catch(() => {});
            if(user) client.logger.error(`Please tell ${user.tag} that their credentials have expired, and that they should /login again. Also tell them that they should fix their perms.`);
        } catch(e) {}
...
const sendDailyShop = async (id, shop, channelId, valorantUser, tryOnOtherShard=true) => {
    ...
    if(!channel) {
        ...
        const user = await client.users.fetch(id).catch(() => {});
        if(user) client.logger.error(`Please tell ${user.tag} that the daily shop is out! (I can't find the channel where the alert was set up)`);
        return;
    }
...
const testAlerts = async (interaction) => {
    try {
        ...
        client.logger.error(`${interaction.user.tag} tried to /testalerts, but failed!`);
        if(e.code === 50013) client.logger.error("Failed with 'Missing Access' error");
        else if(e.code === 50001) client.logger.error("Failed with 'Missing Permissions' error");
...
const loginUsernamePassword = async (interaction, username, password, operationIndex=null) => {
    ...
    if(login.success && user) {
        ...
        client.logger.error(login.error);
        ...
        client.logger.debug(`${interaction.user.tag} login error`);
...
const login2FA = async (interaction, code, operationIndex=null) => {
    ...
    if(login.success && user) {
        ...
        client.logger.error(`${interaction.user.tag} 2FA error`);
        client.logger.error(login.error);
        const index = operationIndex || generateOperationIndex();
        failedOperations.push({
            c: index,
        ...
        client.logger.debug(`${interaction.user.tag} 2FA code failed`);
...
const authFailureMessage = (interactionOrId, authResponse, message="AUTH_ERROR", isEphemeral=false) => {
    ...
    if(authResponse.maintenance) embed = basicEmbed(s(interactionOrId).error.MAINTENANCE);
        ...
        client.logger.debug(`${tag} needs 2FA code`);
        ...
        */
        ...
        client.logger.debug(`${tag} got rate-limited`);
...
const renderAccessoryOffers = async (shop, interaction, valorantUser, KCemoji, id=interaction?.user?.id) => {
        ...
        for (const reward of offer.rewards){
            ...
            switch (reward.ItemTypeID) {
                ...
                case "de7caa6b-adf7-4588-bbd1-143831e786c6": //titles
                    ...
                    break;
                default:
                    client.logger.debug(reward.ItemTypeID);
            }
        }
    }
...
const renderBattlepass = async (battlepass, targetlevel, interaction, targetId=interaction.user.id) => {
...
const skinEmbed = async (uuid, price, interactionOrId, VPemojiString, channel = null) => {
    const skin = await getSkin(uuid);
    const colorMap = {
...
const renderCompetitiveMatchHistory = async (interaction, accountData, matchHistoryData, targetId=interaction.user.id) => { //will be edited in the future
...
const renderProfile = async (interaction, data1, targetId=interaction.user.id) => { //will be edited in the future
    ...
    if(externalAllowed) {
        if(client.config.VALORANT.useEmojisFromServer) {
            try {
                ...
                if(!emojiGuild) client.logger.error("useEmojisFromServer server not found! Either the ID is incorrect or I am not in that server anymore!");
                else {
...
const createEmoji = async (guild, name, filenameOrUrl) => {
    ...
    if(!canCreateEmojis(guild)) return client.logger.debug(`Don't have permission to create emoji ${name} in guild ${guild.name}!`);

    await updateEmojiCache(guild);
    if(guild.emojis.cache.filter(e => !e.animated).size >= maxEmojis(guild))
        return client.logger.debug(`Emoji limit of ${maxEmojis(guild)} reached for ${guild.name} while uploading ${name}!`);
    ...
    try {
        ...
        client.logger.error(`Could not create ${name} emoji in ${guild.name}! Either I don't have the right role or there are no more emoji slots`);
        client.logger.error(`${e.name}: ${e.message}`);
    }
}
...
const importLanguage = (language) => {
    ...
    try {
        ...
        if(language === DEFAULT_LANG) client.logger.error(`Couldn't load ${DEFAULT_LANG}.json! Things will break.`);
        return;
    }
...
const s = (input) => {
    // const discLang = client.config.VALORANT.localiseText ? resolveDiscordLanguage(input) : DEFAULT_LANG;
    const discLang = DEFAULT_LANG;
...
const l = (names, input) => {
    let discLocale = client.config.VALORANT.localiseSkinNames ? resolveDiscordLanguage(input) : DEFAULT_LANG;
...
const handleMQResponse = async (message) => {
    ...
    if(!callbacks[mqid]) return client.logger.error(`No callback registered for MQ response ${message.mqid}!`);
...
const mqProcessRequest = async ({mqid, mqtype, params}) => {
    client.logger.debug("Processing MQ request", mqid, mqtype, JSON.stringify(params).substring(0, 200));
...
const checkRateLimit = (req, url) => {
    ...
    if(rateLimited) {
        ...
        if(retryAfter) {
            client.logger.debug(`I am ratelimited at ${url} for ${retryAfter - 1} more seconds!`);
            if(retryAfter > client.config.VALORANT.rateLimitCap) {
                client.logger.debug(`Delay higher than rateLimitCap, setting it to ${client.config.VALORANT.rateLimitCap} seconds instead`);
                retryAfter = client.config.VALORANT.rateLimitCap;
            }
        }
        else {
            retryAfter = client.config.VALORANT.rateLimitBackoff;
            client.logger.debug(`I am temporarily ratelimited at ${url} (no ETA given, waiting ${client.config.VALORANT.rateLimitBackoff}s)`);
        }
...
const isRateLimited = (url) => {
    ...
    client.logger.debug(`I am still ratelimited at ${url} for ${retryAfter} more seconds!`);
...
const receiveShardMessage = async (message) => {
    ...
    switch(message.type) {
        ...
        case "checkAlerts":
            await checkAlerts();
            break;
        case "client.config.VALORANTReload":
            loadclient.config.VALORANT();
            destroyTasks();
            scheduleTasks();
            break;
        case "skinsReload":
...
const fetch = (url, options={}) => {
    if(client.config.VALORANT.logUrls) client.logger.debug("Fetching url " + url.substring(0, 200) + (url.length > 200 ? "..." : ""));
...
class ValoProxy {
    ...
    createAgent(hostname) {
        ...
        req.on("connect", (res, socket) => {
            client.logger.debug(`Proxy ${this.host}:${this.port} connected to ${hostname}!`);
            if (res.statusCode !== 200) {
                reject(`Proxy ${this.host}:${this.port} returned status code ${res.statusCode}!`);
            }

            socket.on("error", err => {
                client.logger.error(`Proxy ${this.host}:${this.port} socket errored: ${err}`);
    ...
    async test() {
        ...
        if(res.statusCode !== 200) {
            client.logger.error(`Proxy ${this.host}:${this.port} returned status code ${res.statusCode}!`);
            return false;
        }

        const ip = res.body.trim();
        if(!ip) {
            client.logger.error(`Proxy ${this.host}:${this.port} returned no IP!`);
            return false;
        }
...
class ProxyManager {
    ...
    async loadProxies() {
        ...
        for(const line of proxyFile.toString().split("\n")) {
            ...
            if(type !== ProxyType.HTTPS) {
                client.logger.error(`Unsupported proxy type ${type}!`);
    ...
    async loadForHostname(hostname) {
        ...
        for(const proxy of this.allProxies) {
            ...
            const promiseWithTimeout = promiseTimeout(promise, 5000).then(res => {
                if(res === null) {
                    timedOut = true;
                    client.logger.error(`Proxy ${proxy.host}:${proxy.port} timed out!`);
                }
            });
        ...
        if(!activeProxies.length) {
            client.logger.error(`No working proxies found!`);
            return;
        }

        client.logger.debug(`Loaded ${activeProxies.length} proxies for ${hostname}`);
    ...
    async fetch(url, options = {}) {
        ...
        if(req.statusCode === 403 && req.body === "error code: 1020" || checkRateLimit(req, hostnameAndProxy)) {
            client.logger.error(`Proxy ${proxy.host}:${proxy.port} is dead!`);
            client.logger.error(req);
...
const parseSetCookie = (setCookie) => {
    if(!setCookie) {
        client.logger.error("Riot didn't return any cookies during the auth request! Cloudflare might have something to do with it...");
        return {};
    }
...
const ordinalSuffix = (number) => number % 100 >= 11 && number % 100 <= 13 ? "th" : ["th", "st", "nd", "rd"][(number % 10 < 4) ? number % 10 : 0];
...
const transferUserDataFromOldUsersJson = () => {
    ...
    client.logger.debug("Transferring user data from users.json to the new format...");
    client.logger.debug("(The users.json file will be backed up as users.json.old, just in case)");
...
const redeemUsernamePassword = async (id, login, password) => {
    ...
    if(json2.type === 'error') {
        if(json2.error === "auth_failure") client.logger.error("Authentication failure!", json2);
        else client.logger.error("Unknown auth error!", JSON.stringify(json2, null, 2));
...
const redeem2FACode = async (id, code) => {
    ...
    if(json.error === "multifactor_attempt_failed" || json.type === "error") {
        client.logger.error("Authentication failure!", json);
        return {success: false};
    }
...
const processAuthResponse = async (id, authData, redirect, user=null) => {
    ...
    if(rso == null) {
        client.logger.error("Riot servers didn't return an RSO token!");
        client.logger.error("Most likely the Cloudflare firewall is blocking your IP address. Try hosting on your home PC and seeing if the issue still happens.");
...
const fetchRiotClientVersion = async (attempt=1) => {
    ...
    if(error) {
        if(attempt === 3) {
            client.logger.error("Failed to fetch latest Riot user-agent! (tried 3 times)");

            const fallbackVersion = "65.0.2.5073401";
            client.logger.error(`Using version number ${fallbackVersion} instead...`);
        }

        client.logger.error(`Failed to fetch latest Riot user-agent! (try ${attempt}/3`);
        client.logger.error(githubReq);
...
const detectCloudflareBlock = (req) => {
    ...
    if(blocked) {
        client.logger.error("[ !!! ] Error 1020: Your bot might be rate limited, it's best to check if your IP address/your hosting service is blocked by Riot - try hosting on your own PC to see if it solves the issue?")
    }
...
const queueUsernamePasswordLogin = async (id, username, password) => {
    ...
    client.logger.debug(`Added Username+Password login to auth queue for user ${id} (c=${c})`);
...
const queue2FACodeRedeem = async (id, code) => {
    ...
    client.logger.debug(`Added 2fa code redeem to auth queue for user ${id} (c=${c})`);
...
const queueCookiesLogin = async (id, cookies) => {
    ...
    client.logger.debug(`Added cookie login to auth queue for user ${id} (c=${c})`);
...
const queueNullOperation = async (timeout) => {  // used for stress-testing the auth queue
    ...
    client.logger.debug(`Added null operation to auth queue with timeout ${timeout} (c=${c})`);
...
const processAuthQueue = async () => {
    ...
    client.logger.debug(`Processing auth queue item "${item.operation}" for ${item.id} (c=${item.c}, left=${queue.length})`);
    processingCount++;
    ...
    client.logger.debug(`Finished processing auth queue item "${item.operation}" for ${item.id} (c=${item.c})`);
    processingCount--;
}
...
const getWeeklies = async () => {
    client.logger.debug("Fetching mission data...");
...
const getBattlepassProgress = async (interaction, maxlevel, id=interaction.user.id) => {
    const authSuccess = await authUser(id);
    if (!authSuccess.success)
        return authSuccess;

    const user = getUser(id);
    client.logger.debug(`Fetching battlepass progress for ${user.username}...`);
...
const getBattlepassPurchase = async (id) => {
    ...
    const user = getUser(id);
    client.logger.debug(`Fetching battlepass purchases for ${user.username}...`);
...
const renderBattlepassProgress = async (interaction, targetId=interaction.user.id) => {
...
const fetchData = async (types=null, checkVersion=false) => {
    try {
        ...
        client.logger.error("There was an error while trying to fetch skin data!");
        client.logger.error(e);
    }
}

const getSkinList = async (gameVersion) => {
    client.logger.debug("Fetching valorant skin list...");
...
const getPrices = async (gameVersion, id=null) => {
    ...
    user = getUser(id);
    client.logger.debug(`Fetching skin prices using ${user.username}'s access token...`);
...
const getBundleList = async (gameVersion) => {
    client.logger.debug("Fetching valorant bundle list...");
...
const getRarities = async (gameVersion) => {
    ...
    client.logger.debug("Fetching skin rarities list...");
...
const getBuddies = async (gameVersion) => {
    client.logger.debug("Fetching gun buddies list...");
...
const getCards = async (gameVersion) => {
    client.logger.debug("Fetching player cards list...");
...
const getSprays = async (gameVersion) => {
    client.logger.debug("Fetching sprays list...");
...
const getTitles = async (gameVersion) => {
    client.logger.debug("Fetching player titles list...");
...
const fetchBattlepassInfo = async (gameVersion) => {
    client.logger.debug("Fetching battlepass UUID and end date...");
...
const getSkins = async (user) => {
    ...
    if(user.puuid in skinCache) {
        ...
        if(expiresIn <= 0) {
            ...
            client.logger.debug(`Fetched skins collection from cache for user ${user.username}! It expires in ${Math.ceil(expiresIn / 1000)}s.`);
    ...
    client.logger.debug(`Fetched skins collection for ${user.username}`);
...
const getLoadout = async (user, account) => {
    ...
    if(user.puuid in loadoutCache) {
        ...
        if(expiresIn <= 0) {
            ...
            client.logger.debug(`Fetched loadout from cache for user ${user.username}! It expires in ${Math.ceil(expiresIn / 1000)}s.`);
    ...
    client.logger.debug(`Fetching loadout for ${user.username}...`);
    ...
    client.logger.debug(`Fetched loadout for ${user.username}`);
...
const getCache = (user, type) => {
    if(user.puuid in xCache[type]) {
        ...
        const expiresIn = cached.timestamp - Date.now() + client.config.VALORANT.careerCacheExpiration;
        if(expiresIn <= 0) {
            delete xCache[type][user.puuid];
            return {success: false};
        } else {
            client.logger.debug(`Fetched ${type} from cache for user ${user.username}! It expires in ${Math.ceil(expiresIn / 1000)}s.`);
...
const getAccountInfo = async (user, interaction) => {
    ...
    client.logger.debug(`Checked ACCData for ${user.id} R:${accountData?.ratelimits?.remaining} Reset: in ${accountData?.ratelimits?.reset} seconds`)
    const mmrData = await VAPI.getMMRByPUUID({version: "v2",region: user.region, puuid: user.puuid});
    client.logger.debug(`Checked MMRData for ${user.id} R:${mmrData?.ratelimits?.remaining} Reset: in ${mmrData?.ratelimits?.reset} seconds`)
...
const fetchMatchHistory = async (interaction, user, mode="competitive") => {
    ...
    client.logger.debug(`Checked match history for ${user.id} R:${matchHistory?.ratelimits?.remaining} Reset: in ${matchHistory?.ratelimits?.reset} seconds`)
    ...
    if(mode === "competitive") {
        ...
        client.logger.debug(`Checked MMRHistory for ${user.id} R:${mmrHistory?.ratelimits?.remaining} Reset: in ${mmrHistory?.ratelimits?.reset} seconds`)
...
const getShop = async (id, account=null) => {
    ...
    try {
        ...
        client.logger.error("Error adding shop stats!");
        client.logger.error(e);
        client.logger.error(json);
    }
...
const getBalance = async (id, account=null) => {
    ...
    const user = getUser(id, account);
    client.logger.debug(`Fetching balance for ${user.username}...`);
...
const getShopCache = (puuid, target="offers", print=true) => {
    ...
    try {
        ...
        else client.logger.error("Invalid target for shop cache! " + target);

        if(Date.now() / 1000 > expiresTimestamp) return null;

        if(print) client.logger.debug(`Fetched shop cache for user ${discordTag(puuid)}`);
...
const addShopCache = (puuid, shopJson) => {
    ...
    if(!fs.existsSync("src/data/shopCache")) fs.mkdirSync("data/shopCache");
...
module.exports =
{
    ...
    getShopCache,
    fetchShop, fetchBundles, fetchNightMarket, 
    queueNullOperation, 
    discToValLang, discLanguageNames, DEFAULT_LANG, DEFAULT_VALORANT_LANG,
    s, l,  sendMQRequest, sendMQResponse, handleMQRequest, handleMQResponse,
    ...
    getAccountInfo, renderProfile, renderCompetitiveMatchHistory, renderProfile, fetchMatchHistory
}