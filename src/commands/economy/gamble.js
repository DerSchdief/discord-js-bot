...
module.exports = {
  ...
  async interactionRun(interaction) {
    ...
    const response = await gamble(interaction, betAmount);
...
async function gamble(interaction, betAmount) {
  ...
  if (userDb.coins < betAmount)
    return `You do not have sufficient coins to gamble!\n**Coin balance:** ${userDb.coins || 0}${client.config.ECONOMY.CURRENCY}`;
  ...
  const str = `
    **Gamble Amount:** ${betAmount}${client.config.ECONOMY.CURRENCY}
    **Multiplier:** 2x
    ╔══════════╗
  ...
  const result = (reward > 0 ? `You won: ${reward}` : `You lost: ${betAmount}`) + client.config.ECONOMY.CURRENCY;
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(str)
    .setFooter({ text: `${result}\nUpdated Wallet balance: ${userDb?.coins}${client.config.ECONOMY.CURRENCY}` });
